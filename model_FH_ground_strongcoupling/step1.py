...
ground_state = np.load('ground_state.npy')
...
print(trace_distance(noisy_init_state,ground_state))
...
for n in range(num_T):
    ...
    v1 = mycode.trace(kicked_state3.dot(At_Ext)).real
    v2 = mycode.trace(kicked_state4.dot(At_Tro)).real
...
plt.plot(T_axis, Y1_axis,label = 'Re, Exact + Exact')
plt.plot(T_axis, Y2_axis,label = 'Re, Noisy + Trotter')

np.save('T_axis.npy',T_axis)
np.save('signal_Im_ExtExt.npy',O1_axis)
np.save('signal_Im_NoiTro.npy',O2_axis)

np.save('signal_Re_ExtExt.npy',Y1_axis)
np.save('signal_Re_NoiTro.npy',Y2_axis)
